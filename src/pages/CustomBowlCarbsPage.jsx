  ...
  return (
    <div className="page-enter" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '12px 24px 0', display: 'flex', justifyContent: 'flex-end' }}>
        <span style={{ fontSize: '12px', color: '#8A8F84' }}>2 / 4</span>
      </div>
      <div style={{ padding: '8px 24px 0', display: 'flex', gap: '4px' }}>
        {[0, 1, 2, 3].map((i) => (
          <div key={i} style={{ flex: 1, height: '3px', borderRadius: '999px', ...(i <= 1 ? { background: 'linear-gradient(90deg, #8BAA3D, #A0C044)' } : { backgroundColor: 'rgba(139,170,61,0.1)' }) }} />
        ))}
      </div>

      <div style={{ padding: '20px 24px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: '26px', fontWeight: 700, margin: 0, background: 'linear-gradient(135deg, #1B3C2A 0%, #2D5A3F 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Carbs</h1>
      ...
      <div style={{ padding: '4px 24px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '13px', color: '#7A8A6A' }}>Select your carb base</span>
        <span style={{ fontSize: '12px', fontWeight: 600, color: '#1B3C2A', background: 'rgba(139,170,61,0.1)', border: '1px solid rgba(139,170,61,0.15)', borderRadius: '999px', padding: '4px 12px' }}>CHF 4.00</span>
      </div>

      <div style={{ flex: 1, padding: '20px 20px 0', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', alignContent: 'start' }}>
        {carbs.map((item) => {
          const selected = selectedCarbs.includes(item.id)
          return (
            <div key={item.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
              <div onClick={() => toggleCarb(item.id)} style={{
              ...
              <span style={{ fontSize: '10px', color: '#7A7F74' }}>{item.carbs || 0}g carbs</span>
              <button onClick={() => toggleCarb(item.id)} style={{
      ...
      <div style={{ padding: '16px 24px 32px' }}>
        <button onClick={() => canContinue && navigate('/custom/veg')} disabled={!canContinue} style={{
