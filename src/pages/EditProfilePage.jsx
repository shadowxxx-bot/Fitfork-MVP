...
export default function EditProfilePage() {
  ...
  const fieldStyle = (key) => ({
    ...
    borderRadius: '16px', padding: '14px 40px 14px 16px', fontSize: '14px', color: '#1B3C2A',
  ...
  const sliderCSS = `
    ...
    input[type=range].edit-slider::-webkit-slider-track { height: 6px; border-radius: 3px; background: rgba(139,170,61,0.1); }
    input[type=range].edit-slider::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 22px; height: 22px; border-radius: 50%; background: linear-gradient(135deg, #8BAA3D, #A0C044); border: 3px solid white; box-shadow: 0 2px 8px rgba(139,170,61,0.3); cursor: pointer; margin-top: -8px; }
    input[type=range].edit-slider::-moz-range-track { height: 6px; border-radius: 3px; background: rgba(139,170,61,0.1); border: none; }
    input[type=range].edit-slider::-moz-range-thumb { width: 22px; height: 22px; border-radius: 50%; background: linear-gradient(135deg, #8BAA3D, #A0C044); border: 3px solid white; box-shadow: 0 2px 8px rgba(139,170,61,0.3); cursor: pointer; }
  `
  ...
  return (
    <div className="page-enter" style={{ minHeight: '100vh', paddingBottom: '40px' }}>
      ...
      <div style={{ padding: '20px 24px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <button onClick={() => navigate('/profile')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '14px', color: '#B0ADA4', fontWeight: 500, padding: 0 }}>Cancel</button>
        <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: '18px', fontWeight: 700, margin: 0, background: 'linear-gradient(135deg, #8BAA3D, #A0C044)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Edit Profile</h1>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 0 8px' }}>
        <div style={{ position: 'relative' }}>
          <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: 'linear-gradient(135deg, rgba(139,170,61,0.12) 0%, rgba(139,170,61,0.06) 100%)', border: '2px solid rgba(139,170,61,0.3)' }} />
          <div style={{ position: 'absolute', bottom: '-2px', right: '-2px', width: '28px', height: '28px', borderRadius: '50%', background: 'linear-gradient(135deg, #1B3C2A, #2D5A3F)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ padding: '0 24px' }}>
        ...
        <p style={{ fontSize: '11px', fontWeight: 700, color: '#B0ADA4', textTransform: 'uppercase', letterSpacing: '1px', margin: '16px 0 0' }}>Personal information</p>
        ...
        {/* BODY METRICS */}
        <p style={{ fontSize: '11px', fontWeight: 700, color: '#B0ADA4', textTransform: 'uppercase', letterSpacing: '1px', margin: '24px 0 0' }}>Body metrics</p>

        {/* Weight slider */}
        <label style={labelStyle}>Weight</label>
        <div style={{ background: 'rgba(255,255,255,0.72)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.5)', boxShadow: '0 2px 12px rgba(0,0,0,0.04)', borderRadius: '20px', padding: '16px 18px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '14px' }}>
            ...
            <span style={{ fontSize: '16px', fontWeight: 600, color: '#B0ADA4', alignSelf: 'flex-end', paddingBottom: '3px' }}>kg</span>
            <div style={{ display: 'flex', gap: '6px', marginLeft: '6px' }}>
              <button onClick={() => setPf({ ...pf, weight: String(Math.max(20, weightNum - 0.5)) })} style={{ ...btnStyle, background: 'rgba(139,170,61,0.1)', color: '#1B3C2A' }}>−</button>
              <button onClick={() => setPf({ ...pf, weight: String(Math.min(200, weightNum + 0.5)) })} style={{ ...btnStyle, background: 'linear-gradient(135deg, #1B3C2A, #2D5A3F)', color: 'white' }}>+</button>
            </div>
          </div>
          <div style={{ position: 'relative' }}>
            <div style={{
              position: 'absolute', top: '50%', left: 0, right: 0, height: '6px', borderRadius: '3px',
              background: `linear-gradient(to right, #8BAA3D ${((weightNum - 20) / 180) * 100}%, rgba(139,170,61,0.1) ${((weightNum - 20) / 180) * 100}%)`,
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
            <span style={{ fontSize: '10px', color: '#B0ADA4' }}>20 kg</span>
            <span style={{ fontSize: '10px', color: '#B0ADA4' }}>200 kg</span>
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.72)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.5)', boxShadow: '0 2px 12px rgba(0,0,0,0.04)', borderRadius: '20px', padding: '16px 18px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '14px' }}>
            ...
            <span style={{ fontSize: '16px', fontWeight: 600, color: '#B0ADA4', alignSelf: 'flex-end', paddingBottom: '3px' }}>cm</span>
            <div style={{ display: 'flex', gap: '6px', marginLeft: '6px' }}>
              <button onClick={() => setPf({ ...pf, height: String(Math.max(100, heightNum - 1)) })} style={{ ...btnStyle, background: 'rgba(139,170,61,0.1)', color: '#1B3C2A' }}>−</button>
              <button onClick={() => setPf({ ...pf, height: String(Math.min(230, heightNum + 1)) })} style={{ ...btnStyle, background: 'linear-gradient(135deg, #1B3C2A, #2D5A3F)', color: 'white' }}>+</button>
            </div>
          </div>
          <div style={{ position: 'relative' }}>
            <div style={{
              position: 'absolute', top: '50%', left: 0, right: 0, height: '6px', borderRadius: '3px',
              background: `linear-gradient(to right, #8BAA3D ${((heightNum - 100) / 130) * 100}%, rgba(139,170,61,0.1) ${((heightNum - 100) / 130) * 100}%)`,
              transform: 'translateY(-50%)', pointerEvents: 'none', zIndex: 0,
            }} />
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
            <span style={{ fontSize: '10px', color: '#B0ADA4' }}>100 cm</span>
            <span style={{ fontSize: '10px', color: '#B0ADA4' }}>230 cm</span>
          </div>
        </div>
        ...
        {weightNum > 0 && heightNum > 0 && (() => {
          ...
          return (
            <div style={{ background: 'rgba(139,170,61,0.1)', border: '1px solid rgba(139,170,61,0.15)', borderRadius: '16px', padding: '12px 16px', marginTop: '12px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '11px', fontWeight: 700, color: '#B0ADA4', textTransform: 'uppercase', letterSpacing: '1px', margin: '24px 0 0' }}>Nutrition goals per day</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          {[
            ...
            <div key={n.label} style={{ background: 'rgba(255,255,255,0.72)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderRadius: '16px', padding: '14px', border: '1px solid rgba(255,255,255,0.5)', boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}>
          ...
          return (
            <div key={dd.key} style={{ marginTop: '8px' }}>
              <div onClick={() => setOpenDropdown(isOpen ? null : dd.key)} style={{
                ...
                borderBottom: !isOpen ? '1px solid rgba(139,170,61,0.1)' : 'none',
              }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '14px', background: 'rgba(139,170,61,0.1)', border: '1px solid rgba(139,170,61,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{dd.icon}</div>
                <div style={{ flex: 1 }}>
        ...
        <button onClick={handleSave} style={{
          ...
          background: 'linear-gradient(135deg, #8BAA3D, #A0C044)', boxShadow: '0 4px 16px rgba(139,170,61,0.3)',
