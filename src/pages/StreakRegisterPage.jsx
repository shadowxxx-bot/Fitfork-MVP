  ...
  return (
    <div className="page-enter" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '16px 24px' }}>
        <div style={{
          height: '200px', background: 'linear-gradient(135deg, rgba(139,170,61,0.1), rgba(139,170,61,0.05))', border: '1px solid rgba(139,170,61,0.12)', borderRadius: '20px',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: '14px', color: '#7A8A6A', fontWeight: 500,
        }}>Image</div>
      </div>

      {/* Content */}
      <div style={{ flex: 1, padding: '0 24px 32px' }}>
        <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: '24px', fontWeight: 700, background: 'linear-gradient(135deg, #1B3C2A, #8BAA3D)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', margin: '0 0 8px' }}>
          Join FitFork Rewards
        </h1>
        <p style={{ fontSize: '14px', color: '#7A8A6A', margin: '0 0 24px', lineHeight: 1.5 }}>
        ...
        <form onSubmit={handleSubmit} style={{ marginTop: '24px' }}>
          <label style={{ display: 'block', fontSize: '12px', color: '#7A8A6A', fontWeight: 500, marginBottom: '6px' }}>Email</label>
          <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" style={{
            width: '100%', background: 'rgba(255,255,255,0.72)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.5)',
