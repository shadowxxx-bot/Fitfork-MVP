  ...
  return (
    <div style={{ backgroundColor: '#F4F3EC', minHeight: '100vh', paddingBottom: '40px' }}>
      ...
      <div style={{ padding: '20px 24px 0', display: 'flex', alignItems: 'center' }}>
        ...
        <h1 style={{ flex: 1, textAlign: 'center', fontFamily: "'Space Grotesk', sans-serif", fontSize: '18px', fontWeight: 700, color: '#1B3C2A', margin: 0 }}>Help and Support</h1>
        <div style={{ width: '28px' }} />
      </div>

      {/* FAQ */}
      <div style={{ padding: '24px 24px 0' }}>
        <p style={{ fontSize: '12px', color: '#8A8F84', fontWeight: 500, margin: '0 0 12px' }}>Frequently ask question</p>
        <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '4px 0', overflow: 'hidden' }}>
          {faqs.map((faq, i) => {
            const isOpen = openFaq === i
            return (
              <div key={i} style={{
                borderBottom: i < faqs.length - 1 ? '1px solid #EEF2E6' : 'none',
              }}>
                <div onClick={() => setOpenFaq(isOpen ? null : i)} style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  padding: '16px 18px', cursor: 'pointer',
                }}>
                  <span style={{ fontSize: '14px', fontWeight: isOpen ? 600 : 500, color: '#1B3C2A' }}>{faq.q}</span>
                ...
                {isOpen && (
                  <div style={{ padding: '0 18px 16px' }}>
                    <p style={{ fontSize: '13px', color: '#5A7A5A', lineHeight: 1.5, margin: 0 }}>{faq.a}</p>
                  </div>
                )}
              </div>
            )
      ...
      {/* Contact us */}
      <div style={{ padding: '24px 24px 0' }}>
        <p style={{ fontSize: '12px', color: '#8A8F84', fontWeight: 500, margin: '0 0 12px' }}>Contact us</p>
        ...
        <div onClick={() => window.open('tel:+41221234567')} style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <span style={{ fontSize: '11px', color: '#8A8F84' }}>Call the restaurant</span>
          </div>
          <div style={{
            width: '36px', height: '36px', borderRadius: '50%', backgroundColor: '#8BAA3D',
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <span style={{ fontSize: '11px', color: '#8A8F84' }}>support@fitfork.ch</span>
          </div>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#B0ADA4" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="9 18 15 12 9 6" />
          </svg>
      ...
      {/* More */}
      <div style={{ padding: '24px 24px 0' }}>
        <p style={{ fontSize: '12px', color: '#8A8F84', fontWeight: 500, margin: '0 0 12px' }}>More</p>
        <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '4px 0', overflow: 'hidden' }}>
          {[
            ...
            <div key={item.label} style={{
              display: 'flex', alignItems: 'center', gap: '14px',
              padding: '16px 18px',
              borderBottom: i === 0 ? '1px solid #EEF2E6' : 'none',
              cursor: 'pointer',
            }}>
              <div style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                {item.sub && <span style={{ fontSize: '11px', color: '#8A8F84' }}>{item.sub}</span>}
              </div>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#B0ADA4" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <polyline points="9 18 15 12 9 6" />
              </svg>
