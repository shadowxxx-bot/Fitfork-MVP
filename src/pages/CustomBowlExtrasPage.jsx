  ...
  return (
    <div style={{ backgroundColor: '#F5F4EC', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '20px 24px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <button onClick={() => navigate('/custom/veg')} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px', display: 'flex' }}>
      ...
      <div style={{ padding: '8px 24px 0', display: 'flex', gap: '4px' }}>
        {[0, 1, 2, 3].map((i) => (
          <div key={i} style={{ flex: 1, height: '3px', borderRadius: '999px', backgroundColor: '#8BAA3D' }} />
        ))}
      </div>

      <div style={{ padding: '20px 24px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: '26px', fontWeight: 700, color: '#1B3C2A', margin: 0 }}>Extra</h1>
      ...
      <div style={{ padding: '4px 24px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '13px', color: '#5A7A5A' }}>Select your extra</span>
        <span style={{ fontSize: '12px', fontWeight: 600, color: '#1B3C2A', backgroundColor: '#E8EDDA', borderRadius: '999px', padding: '4px 12px' }}>CHF 1.50</span>
      </div>
